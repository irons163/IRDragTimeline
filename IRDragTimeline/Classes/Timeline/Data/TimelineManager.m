...
- (Branch * _Nonnull)demoBranch {
    ...
    Leaf *leaf7 = [[Leaf alloc] initWithData:nil];
    Leaf *leaf8 = [[Leaf alloc] initWithData:nil];
    ...
    Log *log3 = [[Log alloc] initWithLogData:@{@"date":@([NSDate timeIntervalSinceReferenceDate]), @"reason":@"OK", @"rssi":@(202), @"title":@"Connecting..."}];
    [sessionbranch1 setValue:@[log, log2, log3] forKey:@"logs"];
    [branch2 add:leaf6];
    [branch2 add:leaf7];
    [branch2 add:leaf8];
    [sessionbranch2 setValue:@[log, log2, log3] forKey:@"logs"];
    return branch;
}
